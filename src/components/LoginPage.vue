                ...
                <v-flex xs12 sm8 md4>
                  <v-card class="elevation-24" >
                    ...
                    <v-card-text>
                      ...
                      <span class="text-center">Don't have an account</span>
                      <v-btn text color="warning lighten-1" @click="$router.push('/signup')">Sign Up</v-btn>
                    </v-card-text>
                    <v-card-actions>
                      <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    // },
   async login() {
      let result = await axios.get(`https://642d575cbf8cbecdb40381dc.mockapi.io/Users?Name=${this.username}&Password=${this.password}` )
      
      // console.log(this.username, this.password)
      if(result.data.length<1)
      alert("Invalid username or password")
      else{
         if(result.data[0].Name== this.username && result.data[0].Password== this.password){
